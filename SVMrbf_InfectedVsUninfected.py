...
from sklearn.inspection import permutation_importance  # NEW: Import for permutation importance
...
def update_all_plots(n_clicks, infected_hue, uninfected_hue,
    ...
    # Use PCA features as inputs (first n_pcs)
    X_full = adata.obsm["X_pca"][:, :n_pcs]

    # -----------------------------
    # 2) Train/Test Split and Train SVM with RBF kernel
    # -----------------------------
    X_train, X_test, y_train, y_test = train_test_split(X_full, y_full,
                                                        test_size=0.3,
                                                        random_state=42,
                                                        stratify=y_full)
    # Changed kernel from "linear" to "rbf"
    ...
    # -----------------------------
    # 6) Feature Importance Plot via Permutation Importance
    # -----------------------------
    ...
    # Since our features are the first n_pcs principal components,
    # we assign names "PC1", "PC2", ... "PC{n_pcs}"
    feature_names = [f'PC{i+1}' for i in range(n_pcs)]
    importances = perm_results.importances_mean
    # Get the indices of the top 10 features (or fewer if n_pcs < 10)
    ...
    fig_feat = px.bar(
        x=top_features,
        y=top_importances,
        labels={"x": "Principal Component", "y": "Permutation Importance"},
        title="Top 10 Feature Importances (Permutation Importance)"
    )
    ...
    # Changed kernel from "linear" to "rbf" for decision boundary plot
