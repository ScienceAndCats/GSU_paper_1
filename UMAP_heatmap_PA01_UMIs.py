...
def load_and_preprocess_data(file_name, min_counts_cells, min_counts_genes):
    ...
    adata.obs['PA01_count'] = (adata.X[:, pa01_mask] > 0).sum(axis=1)
...
def create_umap_df(adata, n_neighbors, min_dist, n_pcs):
    ...
    umap_df['PA01_count'] = adata.obs['PA01_count']
...
def update_umap(n_clicks, file_name, min_counts_cells, min_counts_genes,
    ...
    fig = px.scatter(
        ...
        y='UMAP2',
        color='PA01_count',
        hover_data=['cell_name', 'leiden', 'PA01_count'],
    ...
    fig.update_layout(
        dragmode='lasso',
        coloraxis_colorbar=dict(title="PA01 Gene Count")
    )
