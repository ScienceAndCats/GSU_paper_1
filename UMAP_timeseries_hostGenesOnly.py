...
# Define the classification function globally for reuse.
...
app.layout = html.Div([
    ...
    html.Br(), html.Br(),
    # New controls for group colors and legend order:
    html.Label("Preinfection Color:"),
    dcc.Input(id="color-preinfection", type="text", value="#00008B"),
    html.Br(),
    html.Label("10min Color:"),
    dcc.Input(id="color-10min", type="text", value="#ff7f0e"),
    html.Br(),
    html.Label("30min Color:"),
    dcc.Input(id="color-30min", type="text", value="#2ca02c"),
    html.Br(),
    html.Label("40min Color:"),
    dcc.Input(id="color-40min", type="text", value="#d62728"),
    html.Br(),
    html.Label("Legend Order (comma separated):"),
    dcc.Input(id="legend-order", type="text", value="Preinfection,10min,30min,40min"),
    html.Br(), html.Br(),
...
# Callback to update UMAP, store data, and show cell counts by group (before and after preprocessing)
@app.callback(
    ...
    State("color-30min", "value"),
    State("color-40min", "value"),
...
def update_umap(n_clicks, file_name, min_counts_cells, min_counts_genes,
                ...
                color_preinfection, color_10min, color_30min, color_40min,
                legend_order):
    ...
    return fig, umap_df.to_json(date_format='iso', orient='split'), raw_data_copy.to_json(date_format='iso',
                                                                                          orient='split'), counts_text
