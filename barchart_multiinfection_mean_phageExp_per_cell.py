...
def mean_gene_expression_per_cell(file_name, prefixes):
    """
    ...
    and for each prefix, calculates the mean gene expression per cell.
    """
    try:
    ...
    # For each prefix, compute the mean gene expression per cell for genes starting with that prefix.
    mean_expr_dict = {}
    for prefix in prefixes:
        matching_genes = [gene for gene in raw_data.columns if gene.startswith(prefix)]
        if matching_genes:
            # Sum expression for matching genes for each cell, then take the mean across all cells.
            mean_expression = raw_data[matching_genes].sum(axis=1).mean()
        else:
            mean_expression = 0
        mean_expr_dict[prefix] = mean_expression
    return mean_expr_dict, None


# Define the app layout.
app.layout = html.Div([
    html.H2("Mean Gene Expression per Cell by Prefix"),
    ...
    html.Br(), html.Br(),
    html.Button("Calculate Mean Expression", id="update-button", n_clicks=0),
    html.Br(), html.Br(),
...
# Callback to load data, calculate mean gene expression per cell, and update the bar chart.
@app.callback(
...
def update_chart(n_clicks, file_name):
    ...
    mean_expr, error = mean_gene_expression_per_cell(file_name, prefixes)
    if error:
        return {}, error

    # Convert the results into a DataFrame for plotting.
    df = pd.DataFrame({
        "Prefix": list(mean_expr.keys()),
        "Mean Expression per Cell": list(mean_expr.values())
    })

    # Create a bar chart.
    fig = px.bar(df, x="Prefix", y="Mean Expression per Cell",
                 title="Mean Gene Expression per Cell by Prefix")
    return fig, ""
